...
<div class="step"><h3>3. Mode and required backups</h3><div class="mode-grid"><label class="mode-option"><input type="radio" name="operation_mode" value="restore" checked><span><strong>Full restore</strong><small>Used in normal operation. Both the database and GLPI files/config backups are required.</small></span></label><label class="mode-option rare"><input type="radio" name="operation_mode" value="fresh"><span><strong>Fresh installation</strong><small>Deletes database, config, files and plugins. Leave both backup fields empty.</small></span></label></div><p class="required-note">Full restore is the default and requires both selections below.</p><div class="row"><div><label>Database backup</label><select name="db_backup_select"><option value="">Select required database backup</option>{% for value,label in db_backups %}<option value="{{ value }}">{{ label }}</option>{% endfor %}</select></div><div><label>GLPI files/config backup</label><select name="file_backup_select"><option value="">Select required GLPI files/config backup</option>{% for value,label in file_backups %}<option value="{{ value }}">{{ label }}</option>{% endfor %}</select></div></div><input type="hidden" name="db_backup_manual"><input type="hidden" name="file_backup_manual"><label class="check"><input type="checkbox" name="skip_plugins" value="yes">Restore without plugins</label><small>GLPI config and files are restored, but plugins, marketplace data and plugin cache are emptied. Database references to plugins may remain.</small></div>
